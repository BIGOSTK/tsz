...
const app_jysz_token = []
let subTitle = ``;
let status;
status = (status = ($.getval("gk_status") || "1") ) > 1 ? `${status}` : ""; // 账号扩展字符
let jysz_token = $.getdata('jysz_token')
...
appyq = process.env.appyq;
    ...
    if(!process.env.jysz_token&&process.env.jysz_token==''){
        console.log(`\n【${$.name}】：未填写相对应的变量`);
        return;
    }
        
    if (process.env.jysz_token && process.env.jysz_token.indexOf('@') > -1) {
        jysz_token = process.env.jysz_token.split('@');
    } else if (process.env.jysz_token && process.env.jysz_token.indexOf('\n') > -1) {
        jysz_token = process.env.jysz_token.split('\n');
    } else if(process.env.jysz_token && process.env.jysz_token.indexOf('#') > -1){
        jysz_token = process.env.jysz_token.split('#');
    }else{
        jysz_token = process.env.jysz_token.split();
    };
    
    Object.keys(jysz_token).forEach((item) => {
        if (jysz_token[item]) {
            app_jysz_token.push(jysz_token[item]);
        };
    });
  ...
  app_jysz_token.push($.getdata('jysz_token'))
...
appyq = $.getdata('appyq');//默认邀请,需要自行修改变量
  ...
  for (let i = 2; i <= jyszcount; i++) {
    app_jysz_token.push($.getdata(`jysz_token${i}`))
    ...
    console.log(`===【共 ${app_jysz_token.length} 个账号】===\n`);
    if(!apptz){apptz=true};
    if(!apptx){apptx=true};
      
for (i = 0; i < app_jysz_token.length; i++) {
    jysz_token=app_jysz_token[i]
    //soy_wx_jysz_headers=soy_wx_jysz_User_Agent
    //JSON.stringify(
    if(!soy_wx_jysz_User_Agent){
        jysz_User_Agent='Mozilla/5.0 (iPhone; CPU iPhone OS 12_5_4 like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) Mobile/15E148 MicroMessenger/8.0.13(0x18000d31) NetType/WIFI Language/zh_CN'
        soy_wx_jysz_headers= {"Host": "apponlie.sahaj.cn","Accept": "application/json","Origin": "http://ppllmm.zhuwentao52.top","User-Agent": "Mozilla/5.0 (iPhone; CPU iPhone OS 12_5_4 like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) Mobile/15E148 MicroMessenger/8.0.13(0x18000d31) NetType/WIFI Language/zh_CN","token": `${jysz_token}`,"X-Requested-With": "com.tencent.mm","Referer": "http://ppllmm.zhuwentao52.top/"}
    }else{
      soy_wx_jysz_headers= {"Host": "apponlie.sahaj.cn","Accept": "application/json","Origin": "http://ppllmm.zhuwentao52.top","User-Agent": `${soy_wx_jysz_User_Agent}`,"token": `${jysz_token}`,
...
function get_appdata() {
   ...
   if ($request.url.indexOf("myInfo") > -1) {
       const jysz_token = $request.headers.token
   if(jysz_token){
       $.setdata(jysz_token,`jysz_token${status}`)
       //$.log(jysz_token)
   }
...
function soy_jysz_task(data) {
    return new Promise((resolve, reject) => {
        $.post({
            ...
            headers : {"Accept": "application/json","Content-Type": "application/json;charset=UTF-8","Host": "apponlie.sahaj.cn","Origin": "http://ppllmm.zhuwentao52.top","Referer": "http://ppllmm.zhuwentao52.top","token": jysz_token,"User-Agent": soy_wx_jysz_User_Agent,"X-Requested-With": "com.tencent.mm"
            },
            body : `${data}`,
        }, async(error, response, data) => {
            //console.log(data)
